...
df_dataset = pd.read_pickle('gs://pipeline-dots-stock/pipeline_root/shkim01/516181956427/ml-with-all-items-20210904153802/get-ml-dataset_-5221492759378001920/ml_dataset')
df_preP = df_dataset.copy()

df_bros = pd.read_pickle('gs://pipeline-dots-stock/pipeline_root/shkim01/516181956427/ml-with-all-items-20210904153802/get-bros_1696036268263079936/bros_univ_dataset')
...
def get_univ_bh02(df, l_dates): # input dataframe : top30s in the period
    ...
    for date in l_dates :
        df_of_the_day = df[df.date == date]
        # df_of_the_day = df_of_the_day.sort_values(by='daily_return', ascending=True)
        # print('df_of_the_date', df_of_the_day)
        df_top30_in_date = df_of_the_day[df_of_the_day.daily_return > 0.05]
        l_top30s_in_date = df_top30_in_date.code.to_list()
        print(f'size of top30 in the date : {df_top30_in_date.shape}')
...
for i in range(idx_start, l_dates.__len__()):
    ...
    dates_for_train = l_dates[i-33: i-3] # 며칠전까지 볼것인가!! 20일만! 일단은
    ...
    df_train = get_univ_bh02(df_preP, dates_for_train)
    df_pred = get_univ_bh02(df_preP, dates_for_pred)
...
# %%
df_pred_all.to_pickle('result_bong04_04.pkl')
# %%

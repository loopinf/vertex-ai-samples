...
def create_model_and_prediction_03(
  ml_dataset : Input[Dataset],
  bros_dataset: Input[Dataset],
  prediction_result_02 : Output[Dataset],
  model_02_artifact: Output[Model],
  ...
  df_ed2 = (
    df_ed2
      .where(lambda df: df.period.isin([60, 90, 120]))
      .dropna(subset=['period'])
  )
  df_ = pd.merge(df_top30_daily, df_ed2,
    left_on = ['date', 'code'],
    right_on = ['date','source'],
    how='left'
  )
  ...
  df_univ = (
    df_.groupby('date')
    .apply(get_univ)
  ).explode().to_frame().rename(columns={0:'code'}).reset_index()

  df_preP = df_univ.merge(df_preP,
    left_on = ['date','code'],
    right_on = ['date','code'],
    how='left')

...
DESC_PIPELINE = 'm18_2_cla_15pct_lp_no_bros_no_top30_05'
...
('Period', int)
    ...
    print(f'data : from {l_dates[0]} to {l_dates[-1]}')
    ...
    def get_top30_bros_dfs_in_period(df, l_dates): # input dataframe : top30s in the period
        ...
        for date in l_dates :
            ...
            df_ = df_of_the_day[(df_of_the_day.change >= -15) & (df_of_the_day.change <= 15)]
...
# create pipeline 
#########################################
job_file_name='gb-model-backtesting-m18-2-15pct-no-bro-top30.json'
@dsl.pipeline(

...
def add_price_on_pattern(date_ref, kernel_size):
  ...
  client = bigquery.Client('dots-stock')
  ...
  def get_pattern(date_ref, kernel_size):
    ...
    if kernel_size in (3, 6):
      ...
      logging.error(f"kernel_size {kernel_size} is not supported")
      raise
    ...
    df = pandas_gbq.read_gbq(
        sql, project_id=PROJECT_ID, use_bqstorage_api=True)
    return df
  ...
  _df3 = \
      (_df2
        ...
        .assign(date_price=lambda df: df.date_price.dt.strftime('%Y-%m-%d'))
  ...
  def to_gbq_table_pattern(df, date_ref):
      schema = [
          ...
          bigquery.SchemaField(
              name="Open", field_type="INT64", mode="REPEATED"),
          bigquery.SchemaField(
              name="High", field_type="INT64", mode="REPEATED"),
          bigquery.SchemaField(
              name="Low", field_type="INT64", mode="REPEATED"),
          bigquery.SchemaField(
              name="Close", field_type="INT64", mode="REPEATED"),
          bigquery.SchemaField(
              name="Volume", field_type="INT64", mode="REPEATED"),
          bigquery.SchemaField(
              name="date_price", field_type="STRING", mode="REPEATED")
      ]

...
def get_ml_op(
    ...
    DESC = "model m19-14 / Classifier based on Top30/ change_p1_over1 / no bros / Incl. top30 only / Excl. KODEX ETN / LongPeriod Training / +- 29% Prediction"
    ...
    # get Univ df
    def get_df_train(df, l_dates): # input dataframe : top30s in the period
    ...
    for i in range(idx_start, l_dates.__len__()):
        ...
        dates_for_train = l_dates[i-43: i-3] # 며칠전까지 볼것인가!! 20일만! 일단은
        date_ref = l_dates[i]

        print(f'train date :  from {dates_for_train[0]} to {dates_for_train[-1]}')
        print(f'prediction date : {date_ref}')

        df_train = get_df_train(df_preP, dates_for_train)

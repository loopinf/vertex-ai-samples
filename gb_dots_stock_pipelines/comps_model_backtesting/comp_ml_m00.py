...
def get_ml_op(
    ...
    DESC = "model m19 is regressor + 15% univ + no_bros_feats (example)"
    ...
    def get_15pct_univ_in_period(df, l_dates): # input dataframe : top30s in the period
        ...
        for date in l_dates :
            ...
            print(f'df_of_the_day : {df_of_the_day.shape}')
            
            df_15pct_of_the_day = df_of_the_day[(df_of_the_day.change >= -15) & (df_of_the_day.change <= 15)]
            print(f'df_15pct_of_the_day : {df_15pct_of_the_day.shape}')
            
            df_univ = df_univ.append(df_15pct_of_the_day)

...
def get_ml_op(
    ...
    DESC = "model m19-13 / Classifier / change_p1_over1 / no bros / Incl top30 / Incl. KODEX ETN / Top40 Training / +- 25% Prediction / mCap_cat"
    ...
    # get Univ df
    def get_top40_univ(df, l_dates): # input dataframe : top30s in the period
        ...
        for date in l_dates :
            ...
            df_of_the_day = df_of_the_day.head(40)
    ...
    for i in range(idx_start, l_dates.__len__()):
        ...
        dates_for_train = l_dates[i-21: i-1] # 며칠전까지 볼것인가!! 20일만! 일단은
        date_ref = l_dates[i]

        print(f'train date :  from {dates_for_train[0]} to {dates_for_train[-1]}')
        print(f'prediction date : {date_ref}')

        df_train = get_top40_univ(df_preP, dates_for_train)
